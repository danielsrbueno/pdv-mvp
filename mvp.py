...
class PDVApp:
    ...
    def init_db(self):
        ...
        self.cursor.execute("""
            CREATE TABLE IF NOT EXISTS usrAdm (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                usuario TEXT UNIQUE,
                senha TEXT
            )
    ...
    def open_admin_panel(self):
        ...
        def delete_selected():
            ...
            self.cursor.execute("DELETE FROM produtos WHERE id = ?", (product["id"],))
            self.conn.commit()
    ...
    def finish_checkout(self, window):
        ...
        # Salva no histórico
        self.cursor.execute(
            "INSERT INTO historico_compras (data, valor_total) VALUES (datetime('now'), ?)",
            (total,)
        )
        self.conn.commit()
